...
const AVATART_ROOT = path.join(__dirname, 'avatart');
const DOWNLOAD_DIR = path.join(AVATART_ROOT, 'downloaded');
const UPLOAD_DIR = path.join(AVATART_ROOT, 'upload');
...
const createCustomer = async (pool, index) => {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      const uploadPath = path.join(UPLOAD_DIR, `${baseName}_upload.jpg`);

      fs.writeFileSync(downloadPath, faceBuffer);
      fs.writeFileSync(uploadPath, createUploadVariant(image));
      ...
      return { name: fullName, downloadPath, uploadPath };
    } catch (error) {
...
const prepareEnvironment = () => {
  if (RESET_MODE && fs.existsSync(AVATART_ROOT)) {
    fs.rmSync(AVATART_ROOT, { recursive: true, force: true });
  }
  ensureDir(AVATART_ROOT);
...
const main = async () => {
  ...
  try {
    ...
    console.log(`Đã hoàn tất tạo ${completed} khách hàng. Dữ liệu đã lưu vào bảng Customers và thư mục ${AVATART_ROOT}.`);
  } finally {
    if (pool) {
      await pool.close();
    }
